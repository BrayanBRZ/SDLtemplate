...
void Camera::Update(float dt)
{
    ...
    if (m_ViewBox.x > m_LimitPosition.x + m_LimitDimension.x * DEFAULT_UNIT_TO_PIXELS * DEFAULT_SCALE - (SCREEN_WIDTH))
    {
        m_ViewBox.x = m_LimitPosition.x + m_LimitDimension.x * DEFAULT_UNIT_TO_PIXELS * DEFAULT_SCALE - (SCREEN_WIDTH);
    }

    if (m_ViewBox.y > m_LimitPosition.y + m_LimitDimension.y * DEFAULT_UNIT_TO_PIXELS * DEFAULT_SCALE - (SCREEN_HEIGHT))
    {
        m_ViewBox.y = m_LimitPosition.y + m_LimitDimension.y * DEFAULT_UNIT_TO_PIXELS * DEFAULT_SCALE - (SCREEN_HEIGHT);
    }
